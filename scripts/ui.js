  ...
  if (loaded) {
    ...
    function startUi() {
      ...
      function startHomeUi() {
        ...
        /*var AQ = canvas.app.HTML.input('range', 40, 500)
        AQ.max = 1000
        AQ.style.width = '250px'
        
        setInterval(function(){
          document.getElementById('p').innerHTML = AQ.value
          var color = ctx.createLinearGradient(30, 0, 30, AQ.value)
          color.addColorStop(0.5, '#6CC144')
          color.addColorStop(0.51, '#B7E867')
          color.addColorStop(1, '#78A233')
        ...
        var ref = new entity({
          type: 'path',
          translate: {
            x: bg.data.x + (bg.data.width / 2) - 20,
            y: bg.data.y + (bg.data.height / 2) - 23
          },
          dWidth: 70,
          dHeight: 30,
          width: 50,
